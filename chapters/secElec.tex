...
\section{Pulse Height Discimination}
\label{sec:PulseHeightDiscrm}
The detector electronics serve as  bridge between the discussion of photons and energy deposition to spectra and their corresponding pulses.
Events that produce a large number of photons will tend to have a higher number of these photons interact with the photocathode of the PMT, thus creating a large electron avalance which results in a larger voltage pulse.

The basis for setting a pulse height discrimiantor to differniate between neutorn and gammas can be attributed to the secondary electrons and the differnece in energy mechanicms as presented in the first section of this chapter.
However, the application to physical detectors has yet to be concretely formualted.
...
Such a pulse height discrminator is usually set by the hardware electronics, but it can also be set by software in an offline analysis.
