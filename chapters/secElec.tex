...
\autoref{sec:ScintMechanics} will provide an overview in order to understand the necessary scintillation mechanics of organic scintillators and as it relates to the energy deposited by charged particles.
\autoref{sec:InteractionsAndRange} will describe the physics of charged particle and gamma interactions in plastics (focusing on polystyrene) in order to explain the basis for the differnece in energy depositions. 
\autoref{sec:EnergyDep} describes the simulation and results of energy deposition in polymeric materials, which results in neutron-gamma discrimination.
...
\begin{figure}
  \centering
    ...
    \begin{subfigure}[b]{0.45\textwidth}
      ...
      \caption[Range of Electrons in Polystryene]{Simulated (GEANT4) range of electrons in polysyrene. Compton scatted electrons from \iso[60]{Co} on the order of  \~ 100 keV will have ranges two orders of magnitude than that of electrons from the charged particle interactions.}
    \end{subfigure}
  \label{fig:ElectronRanges}
\end{figure}

The development of the secondary electron energy ranges so far has been from a simplified treatement of the problem when only the dominant process are considered.
...
\begin{figure}
  \centering
  \includegrahics[width=\textwidth]{ElectronRangeAndEnergyDist}
